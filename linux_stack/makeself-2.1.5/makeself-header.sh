...
# global spinner position holder                                                
SPIN_BUF='|/-\\'
SPIN_CUR='|'                                                                    
...
{                                                                               
    SPIN_CUR=\${SPIN_BUF#?}                                                      
    printf " [%c]  " "\$SPIN_BUF"                                                
    SPIN_BUF=\$SPIN_CUR\${SPIN_BUF%"\$SPIN_CUR"}                                   
    printf "\b\b\b\b\b\b"                                                       
}                                                                               
...
# NRZ - don't wan't to confuse the user.
MS_Help()
{
cat << EOH >&2
Usage: \$0 [OPTION]

Run as root or with sudo.  This will install the package under /opt/gpi.
  $ sudo \$0
...
nox11=$NOX11
copy=$COPY
# NRZ don't let installer change ownership -post hook will set root.
#ownership=y
ownership=n
verbose=n
